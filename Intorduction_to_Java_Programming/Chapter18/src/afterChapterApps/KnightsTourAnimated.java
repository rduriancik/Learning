 ...
 */
public class KnightsTourAnimated extends Application {
    // The image of knight
    ...
    static class BoardPane extends Pane {
        ...
        private int originalKnightX;
        private int originalKnightY;
        private final double SIZE = 25;

        // The image of knight
        private ImagePattern knightImage = new ImagePattern(new Image
                (new File("/home/robert/IdeaProjects/Chapter18/" +
                        "src/afterChapterApps/knightFigure.png").toURI().toString()));
        ...
        private List<Point2D> getAvailablePositions(int knightX, int knightY) {
            ...
            // Tests each possible next position, Point2D objects contains row/column positions of next move in matrix
            if (knightY - 2 >= 0) {
                if (knightX - 1 >= 0
        ...
        protected void findKnightsTour() {
            if (hasKnight()) {
                ...
                if (knightsNextMove != null) {
                    ...
                    if (knightX != originalKnightX || knightY != originalKnightY) {
                        cells[knightX][knightY].setFill(Color.WHITE);
                    }
                    cells[knightsNextMove[0]][knightsNextMove[1]].setFill(knightImage);
                    knightX = knightsNextMove[0];
                    knightY = knightsNextMove[1];

                    Timeline animation = new Timeline(new KeyFrame(Duration.millis(500), event -> findKnightsTour()));
                    animation.play();
                }
            }
        }
